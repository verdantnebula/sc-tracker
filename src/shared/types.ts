...
/**
 * Which tracker the app is showing. The app ships three modes that share the
 * same window, log watcher and DB infrastructure but render different domains:
 *  - cargo   : the original SC Cargo Tracker (default).
 *  - salvage : the SC Salvage Tracker (Drake-Interplanetary themed).
 *  - mining  : the SC Mining reference (MISC industrial blue/white themed).
 * Persisted in settings.json so the chosen mode survives a restart.
 */
export type AppMode = "cargo" | "salvage" | "mining";
...
// ===========================================================================
// Mining reference  (Mining mode — bundled, read-only game reference)
// ---------------------------------------------------------------------------
// Two static game datasets, shipped as a bundled JSON snapshot (no network, no
// token), regenerated by scripts/fetch-mining-reference.mjs from two source
// CSVs. Used by the Mining shell's SCAN LOOKUP / ROCK VALUES / DEPOSITS views.
// ===========================================================================

/**
 * A mineable rock with its six radar SCAN SIGNATURE values (NOT prices). The
 * miner reads a value off the in-game scanner; each of the six is base × 1..6
 * (the tier). `rarity` ∈ Common/Uncommon/Rare/Epic/Legendary.
 */
export interface MiningRock {
  name: string;
  rarity: string;
  /** The six scan-signature values, tier 1..6 (ascending). */
  scanValues: number[];
}

/**
 * A mineable / harvestable material and where it is found. `type` ∈ Ship/Hand/
 * Ground Vehicle Mineable, Harvestable, Creature (plus rarity-qualified
 * variants from the game data). `foundAt` is the parsed location list (may be a
 * single descriptive phrase like "Found in All Deposits").
 */
export interface MiningDeposit {
  name: string;
  type: string;
  foundAt: string[];
}
...
export interface MiningReferenceData {
  rocks: MiningRock[];
